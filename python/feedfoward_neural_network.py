...
TODO(skirmani): Follow Python style guide standards.
"""
...
class FeedfowardNeuralNetwork(neural_network.NeuralNetwork):
    ...
    def printWeights(self):
        for i in range(len(self.weights)):
          print("Weights between layer %s and %s" % (i, i + 1))
    ...
    def train(self, patterns, goal, N=0.4, M=0.1):
        # N: learning rate
        # M: momentum factor
        i = 0
        variables = {}
        while not goal.GoalMet():
            ...
            for p in patterns:
                inputs = p[0]
                targets = p[1]
                variables['outputs'] = self.update(inputs)
                variables['error'] = variables['error'] + self.backPropagate(targets, N, M)
            if i % 100 == 0:
                print('error %-.5f' % variables['error'])
            i += 1
...
def main():
    ...
    # Create goal
    goal = neural_network.ErrorReachedGoal(0.0001)
    # create a network with two input, two hidden, and one output nodes
    n = FeedfowardNeuralNetwork(2, [2, 2], 1)
